...
-   it makes sense only to use data from 1953 onward -since pretty much there is no data available prior to this year
...
graphNr1 <- ggplot(Total_Area_burned_year, aes(x=year, y=Total_Area_million_Ha)) + geom_line(color='red') + ggtitle('Total Area burnt per year - NSW - Million Hectares')

ggplotly(graphNr1)

graphNr2 <- ggplot(Nr_fires_year, aes(x=year, y=n)) + geom_line(color='dark red') + ggtitle('Number of fires per year - NSW')
...
### Q#2: determine if the number and size of fires in the period 1950-1957 are statistically significantly different from the period 1980-2023?
...
period2 <- NSW_fire_history2 %>% filter(year > 1980 & year <= 2023)
